 ...
 */
export class HandlebarTemplateProcessStrategy
  implements TemplateProcessStrategy
{
  ...
  private registerCustomHelpers(): void {
    ...
    this.handlebarsInstance.registerHelper(
      ...
      function (
        ...
        const array = Array.isArray(arrString)
          ? arrString
    ...
    this.handlebarsInstance.registerHelper(
      ...
      function (
        ...
        return ignoreCaseArray.includes(val.toString().toLowerCase())
          ? options.fn(this)
          : options.inverse(this);
      },
    );
    ...
    Logger.info(
      "Handlebars custom helper 함수들이 등록되었습니다: eq, neq, in, startsWith",
    );
  }
  ...
  renderTemplate(template: string, args: ArgumentsType): string {
    try {
      ...
      Logger.info(
        `템플릿 렌더링 성공: ${template.length}자 템플릿, ${Object.keys(args || {}).length}개 변수`,
      );
      return result;
    } catch (error) {
      const errorMessage =
      ...
      Logger.error(
        `Handlebars 템플릿 렌더링 오류: ${errorMessage} - 템플릿: ${template?.substring(0, 100) + (template?.length > 100 ? "..." : "")}, 변수: [${Object.keys(args || {}).join(", ")}]`,
      );
