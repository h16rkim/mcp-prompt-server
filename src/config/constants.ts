...
export const DEFAULT_MESSAGES = {
  ...
  PROMPTS_REGISTERED: (count: number) =>
    `${count}개의 prompts를 MCP prompts로 등록했습니다.`,
  MANAGEMENT_TOOLS_REGISTERED: "관리 도구들을 등록했습니다.",
  PROMPTS_RELOADED: (count: number) =>
    `${count}개의 prompts를 성공적으로 다시 로드했습니다.`,
...
export const ERROR_MESSAGES = {
  ...
  PROMPTS_RELOAD_FAILED: (error: string) =>
    `Prompts 로드 중 오류가 발생했습니다: ${error}`,
  PROMPT_NOT_FOUND: (name: string) =>
    `'${name}' 이름의 prompt를 찾을 수 없습니다.`,
  TEMPLATE_PROCESSING_ERROR: (errors: string[], missingArgs: string[]) =>
    `템플릿 처리 오류: ${errors.join(", ")}. 누락된 필수 인수: ${missingArgs.join(", ")}`,
  INVALID_PROMPT_FORMAT: (file: string) =>
    `경고: ${file}의 Prompt 형식이 올바르지 않습니다. 건너뜁니다.`,
