...
예시:
  ...
  if (!config.promptName) {
    ...
    console.error(
      "사용법: node scripts/e2e-test.js --prompt <name> [--args <json>]",
    );
...
class MCPTester {
  ...
  async testPromptsList() {
    ...
    if (success && response.result?.prompts) {
      this.availablePrompts = response.result.prompts.map(
        (prompt) => prompt.name,
      );
  ...
  async testSinglePrompt(promptName, promptArgs = {}) {
    ...
    try {
      ...
      const success = this.validateResponse(
...
async function main() {
  ...
  const success = await tester.run(
    testConfig.promptName,
    testConfig.promptArgs,
  );
